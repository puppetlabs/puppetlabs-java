...
define java::oracle (
  ...
  case $version {
    '6' : {
      $_release_major = pick($release_major, '6u45')
      $_release_minor = pick($release_minor, 'b06')
      $install_path   = "${java_se}1.${_release_major[0]}.0_${_release_major[2,3]}"
    }
    '7' : {
      $_release_major = pick($release_major, '7u80')
      $_release_minor = pick($release_minor, 'b15')
      $install_path   = "${java_se}1.${_release_major[0]}.0_${_release_major[2,3]}"
    }
    '8' : {
      $_release_major = pick($release_major, '8u51')
      $_release_minor = pick($release_minor, 'b16')
      $install_path   = "${java_se}1.${_release_major[0]}.0_${_release_major[2,3]}"
    }
    default : {
      $_release_major = pick($release_major, '8u51')
